...
"""The Hull-White-Model model: Here the differential equation is given by dr(t)=(b(t)+βr(t))dt +σ dW∗(t).
    """
...
def short_rate_hl_model(b, sigma, initial_r):
    ...
    for i in Brownian_Motion:
        short_rate += [short_rate[-1] + (b[count]+beta*short_rate[-1])/constants.width + sigma * (i-last_b)]
        count += 1
        last_b = i
...
r_path4 = short_rate_hl_model(b, 0, initial_r)
r_path3 = short_rate_hl_model(b, sigma, initial_r)
print(r_path3)
r_path2 = short_rate_hl_model(b, sigma, initial_r)
r_path = short_rate_hl_model(b, sigma, initial_r)
